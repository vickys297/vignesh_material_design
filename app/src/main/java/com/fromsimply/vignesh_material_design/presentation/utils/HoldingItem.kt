...
fun HoldingItem(holding: HoldingUI) {
    ...
    Column(
        ...
        Row {
            Text(holding.symbol, modifier = Modifier.weight(1f))
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Spacer(modifier = Modifier.padding(8.dp))
                Text(
                    holding.ltp.toString(), style = TextStyle(
                        fontWeight = FontWeight.Normal, fontSize = MaterialTheme.typography.titleLarge.fontSize
                    )
                )
            }
        }
...
fun PreviewHoldingUI() {
    HoldingItem(holding = HoldingUI(symbol = "Test Holding", quantity = 1213, ltp = 1213.00, avgPrice = 3432.0, close = 134.1))
}